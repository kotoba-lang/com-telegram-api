(ns telegram-api.main
  "Kotodama WASM entrypoint for the Telegram_api clean-room actor (L4) — Clojure port.
  ...
  facts (`telegram_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "telegram_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "User"       :plural "users"       :id-prefix "telegram_use"
    :fields [:isBot :firstName :lastName :username :languageCode :isPremium] :required [:isBot]
    :coerce {:isBot :bool :isPremium :bool} :refs {}}
   {:entity "Chat"       :plural "chats"       :id-prefix "telegram_cha"
    :fields [:type :title :username :firstName :lastName :isForum] :required [:type]
    :coerce {:isForum :bool} :refs {}}
   {:entity "Message"    :plural "messages"    :id-prefix "telegram_mes"
    :fields [:messageId :date :text :chatId]        :required [:date :text]
    :coerce {:messageId :int :date :int :chatId :int} :refs {:chatId "Chat"}}
   {:entity "ChatMember" :plural "chatmembers" :id-prefix "telegram_cha"
    :fields [:status :isAnonymous :customTitle]          :required [:status :isAnonymous]
    :coerce {:isAnonymous :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"telegram_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "telegram_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :telegram_api-compat/ready)
